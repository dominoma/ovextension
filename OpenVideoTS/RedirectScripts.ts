    ...
    scripts: [{
        ...
        runScopes: [{
            ...
            script: function(details): Promise<VideoTypes.VideoData> {
                ...
                let xmlHttpObj = OV.environment.browser() == OV.environment.Browsers.Chrome ? null : (window as any).XPCNativeWrapper(new (window as any).wrappedJSObject.XMLHttpRequest());
                ...
                function getVideoInfo(): Promise<VideoInfo> {
                    return OV.tools.createRequest({ url: details.url, xmlHttpObj: xmlHttpObj }).then(function(xhr) {
                        ...
                        let urlsHTML = document.querySelectorAll('a[href*="https://www.rapidvideo.com/e/"]') as NodeListOf<HTMLAnchorElement>;
                        let urls: string[] = [];
                    ...
                    if (button == undefined) {
                        ...
                        OV.html.addAttributeListener(button, "className", function() {
                            if (button.className == 'button gray blue') {
                                OV.tools.createRequest({ url: details.url, type: OV.tools.HTTPMethods.POST }).then(function(xhr) {
                                    let HTML = xhr.response;

...
function createVideoLink(videoData : OVPlayer.HistoryEntry) {
    return createLibraryElem(videoData.origin, videoData.title, "https://favicons.githubusercontent.com/"+OV.tools.parseUrl(videoData.origin).host, videoData.poster, true, function(VideoLink){
        OV.storage.local.get(OV.page.getUrlObj().directory).then(function(VideoHashArr : OVPlayer.HistoryEntry[]){
...
OV.page.isReady().then(function() {
    ...
    (document.getElementById("searchText") as HTMLInputElement).placeholder = OV.languages.getMsg("library_site_search_input_lbl");
    ...
    if(!Hash) {
        ...
        OV.storage.local.get(Hash.directory).then(function(VideoHashArr : OVPlayer.HistoryEntry[]){
